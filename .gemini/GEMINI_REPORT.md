...
- **Test Script:** Created `tests/e2e/full_loop.test.ts` to verify the "Ingest -> Control -> Memory -> UI" loop.
    - Note: The test failed with a timeout locally because the services (`ingestion`, `control`, `memory`) need to be running for the loop to work. Please ensure `npm run dev` is running in another terminal before running the test.
...
```bash
npm test tests/e2e/full_loop.test.ts
```
(Ensure infrastructure and apps are running via `npm run infra:up` and `npm run dev` first).
